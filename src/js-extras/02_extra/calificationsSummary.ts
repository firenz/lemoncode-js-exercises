...
const summarizeClassRoom = (studentList: Student[]): StudentSummary[] => {
  // Implementation here
  let summarizedClassRoom: StudentSummary[] = [];
  studentList.forEach(student => {
    let summarizedStudent: StudentSummary = {
      name: student.name,
      highestCalification: Math.max(...student.califications),
      averageCalification: (averageNumber(student.califications)).toPrecision(3)
    };
    summarizedClassRoom.push(summarizedStudent);
  });

  return summarizedClassRoom;
};
...
console.log(`## EXTRAS - EXTRA - Califications summary ##
Students: ${JSON.stringify(students)}
Summarized students: ${JSON.stringify(summarizeClassRoom(students))}`);
